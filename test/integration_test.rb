...
class IntegrationTest < Minitest::Test
  ...
  def test_round_trip_build_and_extract
    ...
    File.write(File.join(gem_dir, "lib/fakegem/fakegem.#{ext}"), binary_content)
    ...
    extension_dir = File.join(@tmpdir, "consumer_extensions")
    FileUtils.mkdir_p(extension_dir)

    consumer_spec = mock("spec")
    consumer_spec.stubs(:extension_dir).returns(extension_dir)

    Prebake::Extractor.install(platform_gem_path, consumer_spec)

    # 4. Verify the binary was extracted correctly
    extracted = Dir.glob(File.join(extension_dir, "**/*.#{ext}")).first
    assert extracted, "Expected to find extracted .#{ext} file"
